...
CSV_VAL        = "val.csv"
CSV_TEST       = "test.csv"
CHECKPOINT_DIR = "checkpoints_20_resnest50d_4s2x40d"
METRICS_CSV    = "training_metrics_20_resnest50d_4s2x40d.csv"
NUM_EPOCHS     = 20
BATCH_SIZE     = 32
...
if hasattr(model, 'fc'):
    model.fc = nn.Sequential(
        nn.Dropout(0.5),
        nn.Linear(model.fc.in_features, 19),
        nn.Sigmoid(),
    )
elif hasattr(model, 'head'):  # for swin models
    model.head = nn.Sequential(
        nn.Dropout(0.5),
        nn.Linear(model.head.in_features, 19),
        nn.Sigmoid(),
    )
